...
class AutoEagerLoadingTest extends \ORMTestCase
{
	...
	public function testAutoEagerLoading()
	{
		...
		$this->assertEquals('SELECT * FROM "articles" WHERE "auto_eager_loading_user_id" IN (\'1\', \'2\', \'3\')', $this->connectionManager->connection('sqlite')->getLog()[1]['query']);
	}
	...
	public function testDisableAutoEagerLoading()
	{
		...
		$this->assertEquals('SELECT * FROM "articles" WHERE "auto_eager_loading_user_id" = \'1\'', $this->connectionManager->connection('sqlite')->getLog()[1]['query']);

		$this->assertEquals('SELECT * FROM "articles" WHERE "auto_eager_loading_user_id" = \'2\'', $this->connectionManager->connection('sqlite')->getLog()[2]['query']);

		$this->assertEquals('SELECT * FROM "articles" WHERE "auto_eager_loading_user_id" = \'3\'', $this->connectionManager->connection('sqlite')->getLog()[3]['query']);
	}
}
